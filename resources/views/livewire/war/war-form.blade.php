<div>
    <table class="table table-bordered table-sm war-table">
        <thead>
            <tr>
                <th class="text-center align-middle px-0" style="width:130px;">
                ...
                <th class="align-top" colspan="3" style="width:220px;">
                    DATE SUBMITTED: <br>
    ...
    <div class="table-responsive">
        ...
        @if(!empty($area_lines))
            <table class="table table-bordered table-sm war-table">
                ...
                <thead>
                    <tr>
                        <th class="align-middle war-label" colspan="5">II. AREAS</th>
                    </tr>
                </thead>
                                ...
                                <tr class="text-center">
                                    <td class="p-0 px-1 align-middle">
                                        <div class="d-flex align-items-start flex-column">
                                            <div class="branch-name mb-0" style="font-size:0.82rem;line-height:1.2;">
                                                <span class="badge badge-{{$schedule['source'] == 'request' ? 'warning' : 'success'}} mr-1">{{$schedule['source']}}</span>
                                            ...
                                            @if(!empty($line['attachments_arr'][$line['date']][$schedule['branch_id']]))
                                                <div class="attachment-list mt-0" style="font-size:0.76rem;">
                                                    @foreach($line['attachments_arr'][$line['date']][$schedule['branch_id']] as $attachments)
                                                        @if(!empty($attachments))
                                                            @foreach($attachments as $attach)
                                                                <a href="{{asset('storage/'.$attach['file'])}}" target="_blank">{{$attach['title']}}</a>
                                            ...
                                            <div class="mt-1">
                                                @if(isset($attachment_view['schedule'][$schedule['id']]) && $attachment_view['schedule'][$schedule['id']])
                                                    <div class="input-group input-group-sm">
                                                        <input type="file" class="form-control form-control-sm" name="branch_attachment[{{$line['date']}}][{{$schedule['branch_id']}}][]" multiple form="{{$type}}">
                                                        <span class="input-group-append">
                                                            <button class="btn btn-xs btn-danger" wire:click.prevent="showAttachments({{$schedule['id']}}, 'schedule')">✕</button>
                                                        </span>
                                                    </div>
                                                @else
                                                    <button class="btn btn-xs btn-outline-secondary" wire:click.prevent="showAttachments({{$schedule['id']}}, 'schedule')">
                                                        <i class="fa fa-paperclip mr-1"></i>Attach
                                                    </button>
                                                @endif
                                            </div>
                                        </div>
                                    ...
                                    <td class="p-0">
                                        <div class="input-group input-group-sm">
                                            ...
                                            @endphp
                                            <textarea name="action_points[{{$line['date']}}][{{$schedule['branch_id']}}]" class="form-control border-0 bg-editable align-middle" form="{{$type}}" rows="2">{{$action_point ?? ''}}</textarea>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr class="text-center">
                                        ...
                                        <div class="d-flex align-items-start flex-column">
                                            ...
                                            <div class="mt-1">
                                                @if(isset($attachment_view['deviation'][$deviation['id']]) && $attachment_view['deviation'][$deviation['id']])
                                                    <div class="input-group input-group-sm">
                                                        <input type="file" class="form-control form-control-sm" name="branch_attachment[{{$line['date']}}][{{$deviation['branch_id']}}][]" multiple form="{{$type}}">
                                                        <span class="input-group-append">
                                                            <button class="btn btn-xs btn-danger" wire:click.prevent="showAttachments({{$deviation['id']}}, 'deviation')">✕</button>
                                                        </span>
                                                    </div>
                                                @else
                                                    <button class="btn btn-xs btn-outline-secondary" wire:click.prevent="showAttachments({{$deviation['id']}}, 'deviation')">
                                                        <i class="fa fa-paperclip mr-1"></i>Attach
                                                    </button>
                                                @endif
                                            </div>
                                    ...
                                    <td class="p-0">
                                        <div class="input-group input-group-sm">
                                            ...
                                            @endphp
                                            <textarea name="action_points[{{$line['date']}}][{{$deviation['branch_id']}}]" class="form-control border-0 bg-editable align-middle" form="{{$type}}" rows="2">{{$action_points ?? ''}}</textarea>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
    ...
    <table class="table table-bordered table-sm war-table">
        <tr>
            <th class="align-middle war-label" colspan="5">III. Highlight(s) of week's field visit (use 2nd page for more highlights when necessary):</th>
        </tr>
        <tr>
